...
class Binding<T> {
    ...
    public function new(module : Module) {
        if (module.toBool().not()) Funk.error(ArgumentFail());
    ...
    public function to(type : Class<T>, ?func : Function0<Array<Dynamic>>) : Scope {
        if (type.toBool().not()) Funk.error(ArgumentFail());
        if (func.toBool().not()) {
            func = function () {
                return [];
            };
    ...
    public function toInstance(instance : T) : Scope {
        if (instance.toBool().not()) Funk.error(ArgumentFail());
    ...
    public function toProvider(provider : Class<Provider<T>>) : Scope {
        if (provider.toBool().not()) Funk.error(ArgumentFail());
