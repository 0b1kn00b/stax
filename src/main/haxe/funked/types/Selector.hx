    ...
    public function execute() : List<Expr> {
        ...
        while(_lexer.hasNext()) {
            ...
            if (AnyTypes.toBool(expr)) {
                if (_bracket != 0) {
                    Funk.error(IllegalOperationFail("Bracket mismatch; extra left ( found."));
                }
    ...
    private function matchToken(opt : Option<Token>) : Expr {
        ...
        var openBlock = function () {
            ...
            var token = if (hasNext()) {
                ...
                Funk.error(IllegalOperationFail("Exhausted"));
            }
            return ESub(token);
        };
        var closeBlock = function () {
            _bracket--;
            if (_bracket < 0) {
                Funk.error(IllegalOperationFail("Bracket mismatch; extra right ) found."));
            }
            return null;
        };
        ...
        return switch (opt) {
            case Some(token):
                switch(token){
                    ...
                    case Unknown: Funk.error(IllegalOperationFail("Unknown token"));
                    case _: null;
                }
            case _: null;
        }
