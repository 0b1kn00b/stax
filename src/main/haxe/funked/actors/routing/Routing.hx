...
class Router extends Actor {
    ...
    public function new() {
        ...
        _ref = switch(c) {
            ...
            case _: Funk.error(ActorFail('Router actor can only be used in RoutedActorRef, not in ${c}'));
        }
    }
    ...
    override public function preRestart(reason : Fails, message : Option<AnyRef>) : Void {}
...
class NoRouter implements RouterConfig {
    ...
    public function createRoute(routeeProvider : RouteeProvider) : Route {
        return Funk.error(ActorFail("NoRouter does not createRoute"));
    }

    public function createActor() : Actor return Funk.error(ActorFail("NoRouter does not createActor"));

    public function createRouteeProvider(context : ActorContext, routeeProps : Props) : RouteeProvider {
        return Funk.error(ActorFail("NoRouter does not createRouteeProvider"));
    }
