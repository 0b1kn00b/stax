...
class SupervisorStrategy {
    ...
    public function processFail( context : ActorContext,
                                    restart : Bool,
                                    child : ActorRef,
                                    cause : Dynamic,
                                    stats : ChildStats,
    ...
    public function handleFail(  context : ActorContext,
                                    child : ActorRef,
                                    cause : Dynamic,
                                    stats : ChildStats,
                                    children: List<ChildStats>
        ...
        logFail(context, child, cause, directive);

        return switch(directive) {
            case Resume: resumeChild(child, cause); true;
            case Restart: processFail(context, true, child, cause, stats, children); true;
            case Stop: processFail(context, false, child, cause, stats, children); true;
            case _: false;
        }
    }
    ...
    private function logFail(context : ActorContext, child : ActorRef, cause : Dynamic, directive : Strategies) : Void {
        ...
        switch (directive) {
            ...
            case _: publish(context, Fail, FailMessage(cause, child.path().toString(), Type.getClass(this), logMessage));
        }
    }
...
class OneForOneStrategy extends SupervisorStrategy {
    ...
    override public function processFail(    context : ActorContext,
                                                restart : Bool,
                                                child : ActorRef,
                                                cause : Dynamic,
                                                stats : ChildStats,
