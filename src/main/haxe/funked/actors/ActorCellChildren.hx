...
class Children {
    ...
    private function resumeChildren(causedByFail : Dynamic, perp : ActorRef) : Void {
        _container.children().foreach(function(child) {
            AnyTypes.asInstanceOf(child, InternalActorRef).resume((perp == child) ? causedByFail : null);
        });
    }

    private function checkName(name : String) : String {
        return switch(name) {
            case _ if(name == null): Funk.error(ArgumentFail("actor name must not be null"));
            case _ if(name == ""): Funk.error(ArgumentFail("actor name must not be empty"));
            case _ if(ActorPathName.NameRegexp.match(name)): name;
            case _: Funk.error(ArgumentFail('illegal actor name "$name"'));
        }
    }
...
class NormalChildrenContainer implements ChildrenContainer {
    ...
    public function reserve(name : String) : ChildrenContainer {
        if(_map.exists(name)) Funk.error(ArgumentFail('actor name $name is not unique!'));
...
class TerminatingChildrenContainer implements ChildrenContainer {
    ...
    public function reserve(name : String) : ChildrenContainer {
        return Funk.error(ActorFail('cannot reserve actor name ${name} already terminating'));
    }
...
class TerminatedChildrenContainer implements ChildrenContainer {
    ...
    public function add(name : String, child : ActorRef) : ChildrenContainer {
        return Funk.error(ActorFail('cannot reserve actor name ${name} already terminated'));
    }

    public function remove(child : ActorRef) : ChildrenContainer {
        return Funk.error(ActorFail('cannot reserve actor name ${child.path().name()} already terminated'));
    }
