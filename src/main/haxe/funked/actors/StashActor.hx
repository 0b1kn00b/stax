...
class StashActor extends Actor {
    ...
    public function new() {
        ...
        else Funk.error(ActorFail('DequeBasedMailbox required, got: ${Type.getClass(messageQueue)}'));
    }

    public function stash() : Void {
        var currentMessage = context().asInstanceOf(ActorCell).currentMessage();
        if (_stash.size() > 0 && (_stash.head() == currentMessage)) {
            Funk.error(IllegalOperationFail('Can\'t stash the same message $currentMessage more than once'));
        }

        if (_capacity <= 0 || _stash.size() < _capacity) _stash = _stash.prepend(currentMessage);
        else Funk.error(ActorFail('Couldn\'t enqueue message $currentMessage to stash of ${context().self()}'));
    }
