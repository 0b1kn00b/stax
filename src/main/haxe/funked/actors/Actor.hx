...
class Actor implements ActorRef {
    ...
    public static var emptyBehaviour : Receive = PartialFunction1Types.fromPartial(Partial1(function(x) return false, 
                                                                                            function(x) return Funk.error(IllegalOperationFail("Empty behavior apply()"))
                                                                                            ));
    ...
    public function new() {
        ...
        if (context.isEmpty()) Funk.error(ActorFail("No Context Fail"));
    ...
    public function preRestart(reason : Fails, message : Option<AnyRef>) : Void {
        _context.children().foreach(function(child) child.context().stop());
        postStop();
    }

    public function postRestart(reason : Fails) : Void preStart();
