...
interface Cell extends ActorContext {
    ...
    function resume(causedByFail : Dynamic) : Void;
...
class ActorCell implements Cell implements ActorContext {
    ...
    public function resume(causedByFail : Dynamic) : Void {
        try _dispatcher.systemDispatch(this, Resume(causedByFail)) catch (e : Dynamic) handleException(e);
    }
    ...
    public function systemInvoke(message : SystemMessage) : Void {
        try {
            switch(message) {
                ...
                case Resume(inResponseToFail): systemResume(inResponseToFail);
                case Watch(watchee, watcher): addWatcher(watchee, watcher);
                case Unwatch(watchee, watcher): remWatcher(watchee, watcher);
                case _: // got to catch'em all.
            }
        } catch (e: Dynamic) {
            handleInvokeFail(Nil, e);
        }
    }
    ...
    private function autoReceiveMessage(message : EnvelopeMessage) : Void {
        ...
        switch(msg) {
            case Failed(cause, uid): handleFail(sender().getOrElse(function() return null), cause, uid);
            case Terminated(t) : watchedActorTerminated(t);
            case Kill: Funk.error(ActorKillFail("Kill"));
    ...
    private function newActor() : Actor {
        ...
        try {
            ...
            if (!AnyTypes.toBool(instance)) {
                Funk.error(ActorFail("Actor instance passed to actorOf can't be 'null'"));
            }
            ...
            publish(Fail, FailMessage(e, self().path().toString(), null, 'unable to create new actor'));
            throw e;
        }
    ...
    private function systemCreate(uid : String) : Void {
        ...
        try {
            ...
            publish(Fail, FailMessage(e, self().path().toString(), null, 'unable to create system ${uid}'));
            throw e;
        }
    }
    ...
    private function systemResume(inResponseToFail : Dynamic) : Void {
        switch(_children.waitingForChildren()) {
            case None: faultResume(inResponseToFail);
            case _: // TODO (Simon) : Implement waiting children.
        }
    }
    ...
    private function resumeChildren(causedByFail : Dynamic, perp : ActorRef) : Void {
        _children.resumeChildren(causedByFail, perp);
    }

    private function handleFail(child : ActorRef, cause : Dynamic, uid : String) {
        switch(_children.getChildByRef(child)) {
            case Some(stats) if (AnyTypes.toBool(_actor) && ChildStatsTypes.uid(stats) == uid):
                if (!_actor.supervisorStrategy().handleFail(this, child, cause, stats, _children.getAllChildStats())) {
                    throw cause;
                }
            case Some(stats):
                #if debug
    ...
    private function handleChildTerminated(child : ActorRef) : Void {
        ...
        if (AnyTypes.toBool(_actor)) {
            try _actor.supervisorStrategy().handleChildTerminated(this, child, _children.children()) catch(e : Dynamic) {
                publish(Fail, FailMessage(e, _self.path().toString(), Type.getClass(_actor), 'handleChildTerminated failed'));
                handleInvokeFail(Nil, e);
            }
        }
    ...
    private function faultRecreate(cause : Dynamic) : Void {
        if (!AnyTypes.toBool(_actor)) {
            publish(Fail, FailMessage(    cause,
                                            _self.path().toString(),
                                            Type.getClass(_actor),
            ...
            if (AnyTypes.toBool(failedActor)) {
                ...
                try {
                    if (AnyTypes.toBool(failedActor.context())) failedActor.preRestart(asFail(cause), optionalMessage);
                } catch (e : Dynamic) {
                    publish(Fail, FailMessage(e, _self.path().toString(), Type.getClass(_actor), Std.string(e)));
                }

                if (!_mailbox.isSuspended()) {
                    Funk.error(ActorFail('mailbox must be suspended during restart, status=${_mailbox.status()}'));
                }
    ...
    private function asFail(error : Dynamic) : Fails {
        return if (error == null) Fail(Std.string(error));
        else if (AnyTypes.isValueOf(error, Fails)) error;
        else Fail(error);
    }

    private function faultResume(causedByFail : Dynamic) : Void {
        var e = causedByFail;
        if (!AnyTypes.toBool(_actor)) {
            publish(Fail, FailMessage(e, _self.path().toString(), null, 'changing Resume into Create after $e'));
            faultCreate();
        } else if (!AnyTypes.toBool(_actor.context()) && !AnyTypes.toBool(causedByFail)) {
            publish(Fail, FailMessage(    e,
                                            _self.path().toString(),
                                            Type.getClass(_actor),
                                            'changing Resume into Restart after $e'
                                            ));
            faultRecreate(causedByFail);
        } else {
            var perp = perpetrator();
            try resumeNonRecursive() catch(e : Dynamic) {}
            if (causedByFail != null) clearFailed();
            resumeChildren(causedByFail, perp);
        }
    }

    private function faultCreate() : Void {
        if (!_mailbox.isSuspended()) {
            Funk.error(ActorFail('mailbox must be suspended during creation, status=${_mailbox.status()}'));
        }
    ...
    private function finishRecreate(cause : Dynamic, failedActor : Actor) : Void {
        ...
        try {
            ...
            freshActor.postRestart(asFail(cause));
            ...
            clearActorFields(_actor);
            handleInvokeFail(survivors, new PostRestartException(_self, e, cause));
        }
    }

    private function handleInvokeFail(childrenNotToSuspend : List<ActorRef>, error : Dynamic) : Void {
        #if debug
    ...
    private function handleException(e : Dynamic) : Void {
        ...
        switch(e){
            case _: publish(Fail, FailMessage(    e,
                                                    _self.path().toString(),
                                                    Type.getClass(_actor),
...
private class TerminatatedActorProps extends Props {
    ...
    override public function creator() : Function0<Actor> {
        return Funk.error(IllegalOperationFail("This Actor has been terminated"));
    }
}
