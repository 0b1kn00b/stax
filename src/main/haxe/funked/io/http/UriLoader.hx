...
class UriLoader<T> {
    ...
    public function new(request : UriRequest, map : Function1<String, T>) {
        ...
        var url = switch(request.url()) {
            ...
            case _: Funk.error(HttpFail("Invalid UriRequest"));
        };
        ...
        _loader.onStatus = function (status : Int) {
            ...
            if (status == 0) _deferred.reject(HttpFail('SecurityFail: "Same Origin Policy" at "${_request.uri()}"'));
        ...
        _loader.onFail = function (error : String) {
            _statusStream.dispatch(Some(HttpClientFail(Fail)));
            _deferred.reject(HttpFail('$error for url ${_request.uri()}'));
        };
    }
    ...
    public function start(method : HttpMethod) : Promise<HttpResponse<T>> {
        ...
        var request = switch (method) {
            ...
            case _: Funk.error(IllegalOperationFail('${Std.string(method)} not supported'));
        };
