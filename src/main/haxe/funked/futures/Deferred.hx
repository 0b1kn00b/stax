...
class Deferred<T> {
        ...
        return switch(_values.last()) {
            case Some(state):
                switch(state) {
                    ...
                    case Rejected(error): Fail(error);
                    case _: Fail(NoSuchElementFail);
                }
            case _: Fail(NoSuchElementFail);
        }
    }
            ...
            case Some(state):
                switch(state) {
                    case Pending: 
                        ...
                        value;
                    case _: Funk.error(IllegalOperationFail('Invalid state'));
                }
            case _: Funk.error(IllegalOperationFail('Invalid value'));
        }
    }
    ...
    public function reject(error : Fails) : Fails {
        return switch(_values.last()) {
            case Some(state):
                switch(state) {
                    case Pending: 
...
private class PromiseImpl<T> implements Promise<T> {
    ...
    private var _but : Signal1<Fails>;
        ...
        _stateStream.foreach(function(value) {
            ...
            switch(value) {
                ...
                case Rejected(error):
                    ...
                    _but.dispatch(error);
                    _when.dispatch(Fail(error));

                case Aborted:

                    _when.dispatch(Fail(NoSuchElementFail));
    ...
    public function but<R>(func : Function1<Fails, R>) : Promise<R> {
        var deferred = new Deferred();
        var promise = deferred.promise();
        switch(_state){
    ...
    public function when<R>(func : Function1<Attempt<T>, R>) : Promise<R> {
        ...
        switch(_state){
            ...
            case Rejected(value): deferred.reject(cast func(Fail(value)));
            case _:
        }
        return promise;
    }
...
class EmptyPromise<T> implements Promise<T> {
    ...
    public function but<R>(func : Function1<Fails, R>) : Promise<R> return new Deferred().promise();
