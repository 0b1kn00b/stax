...
interface Promise<T> {
    ...
    function but<R>(func : Function1<Fails, R>) : Promise<R>;
...
class PromiseTypes {
    ...
    public static function reject<T>(value : String) : Promise<T> {
        var deferred = new Deferred();
        var promise = deferred.promise();
        deferred.reject(ActorFail(value));
        return promise;
    }
    ...
    public static function pipe<T>(promise : Promise<T>, deferred : Deferred<T>) : Promise<T> {
        ...
        promise.when(function(attempt) {
            switch (attempt) {
                case Fail(error): deferred.reject(error);
