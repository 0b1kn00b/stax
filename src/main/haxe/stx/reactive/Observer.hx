...
abstract Observer<T>(IObserver<T>) from IObserver<T> to IObserver<T>{
  static public function create<T>(?fn0:T->Void,?fn1:Fail->Void,?fn2:Niladic):Observer<T>{
    return new DefaultObserver(fn0,fn1,fn2);
  }
  static public function unit<T>(){
    return new AnonymousObserver(function(chk:Chunk<T>){});
  }
