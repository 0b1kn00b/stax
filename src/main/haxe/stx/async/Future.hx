...
abstract Future<T>(FutureType<T>) from FutureType<T> to FutureType<T>{
  public inline function new(f:FutureType<T>){
    this = f;
  }
  /*@:from static public inline function fromPromise<A>(prm:Promise<A>):Future<stx.Outcome<A>>{
    return new Future(prm);
  }*/
  @:to public function toFunction():(T->Void)->Void{
    return Continuations.apply.bind(this);
  }
  ...
  public function timeout(deliver:Thunk<T>,after:Float):Future<T>{
    return Futures.timeout(this,deliver,after);
  }
}
class Futures{
  ...
  static public function bindFold<A,B>(arr:Array<A>,init:B,fold:B->A->Future<B>){
    return stx.Arrays.foldLeft(arr,
      Future.pure(init),
      function(memo:Future<B>,next:A):Future<B>{
        return memo.flatMap(
          function(b:B){
            return fold(b,next);
          }
  ...
  static public function wait<T>(arr:Array<Future<T>>):Future<Array<T>>{
  ...
  static public function timeout<T>(ft:Future<T>,deliver:Thunk<T>,after:Float){
