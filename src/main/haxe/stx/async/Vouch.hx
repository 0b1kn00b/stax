...
abstract Vouch<T>(Eventual<Chunk<T>>) from Eventual<Chunk<T>> to Eventual<Chunk<T>>{
  ...
  @:from static public function fromContract<A>(prm:stx.async.Contract<A>):Vouch<A>{
    return prm.asEventual().map(Outcomes.toChunk);
  }
  @:from static public function fromChunk<A>(chk:Chunk<A>):Vouch<A>{
    return stx.async.Eventual.pure(chk);
  }
  @:from static public function fromOption<A>(m:Option<A>):Vouch<A>{
...
class Vouches{
  ...
  static public function bindFold<A,B>(it:Array<A>,start:B,fm:B->A->Vouch<B>):Vouch<B>{
    return stx.async.Eventuals.bindFold(
      it,
      Val(start),
  ...
  static public function toContract<A>(vch:Vouch<A>):stx.async.Contract<A>{
    var p : stx.async.Contract<A> = Contracts.toContract(fold(vch,
        Success,
...
class Vouches1{
  ...
  */
  ...
  static public function toVouch<A>(p:stx.async.Contract<A>):Vouch<A>{
