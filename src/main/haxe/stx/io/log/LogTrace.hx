...
class LogTrace{
  public static dynamic function trace( v : Dynamic ) : Void {
    #if flash
      #if (fdb || native_trace)
      ...
      #else
        untyped FlashLogTrace.__trace(v);
      #end
    #elseif neko
      untyped {
        $print(v);
    ...
    #elseif js
      //untyped JavascriptLogTrace.__trace(v);
    #elseif php
        untyped __php__("
...
class FlashLogTrace{
  ...
  public static function __trace(v : Dynamic) {
    var tf = flash.Boot.getTrace();
    if( lines == null ) lines = [];
    var str = __string_rec(v, "");
